...
<p align="right">
  ...
  <a href="https://girke.bioinformatics.ucr.edu/GEN242/slides/rprogramming/rprogramming_page.html" target="_blank" rel="noopener noreferrer" style="text-decoration:none">
    <img src="https://img.shields.io/badge/Slideshow-blue?style=for-the-badge" alt="Slideshow">
  </a>
  <a href="https://raw.githubusercontent.com/tgirke/GEN242/main/tutorials/rprogramming/rprogramming_index.qmd" target="_blank" rel="noopener noreferrer" style="text-decoration:none">
...
quarto render rprogramming_index.qmd
Render qmd from R:
quarto::quarto_render("rprogramming_index.qmd")
-->
...
The previous [Rbasics](../rbasics/rbasics_index.qmd) tutorial provides a general introduction to the usage of the R environment and its basic command syntax.
...
![R_Interfaces](images/rstudio.png){fig-align="center"}![vim-r](images/vimR.png){fig-align="center"}
...
R programming involves the writing of R scripts, which are text files containing R code. The file name of an R script carries the extension `.R` (_e.g._ `myscript.R`). Typically, R scripts are composed of code sections, as well as comments (starting with `#`) explaining what the code does. 
...
__Programming Exercise on Loops:__ go [here](../rprogramming/index.qmd#exercise-1)
...
```{r usage_function_example3}
myfct(x1=2) 

```
Print function definition (often unintended) 

```{r usage_function_example4}
myfct 

```

__Programming Exercise on Functions:__ go [here](../rprogramming/index.qmd#exercise-2)
../rprogramming/index.qmd#exercise-1)
...
A [shebang](https://en.wikipedia.org/wiki/Shebang_(Unix)) needs to be included at the beginning of an R script in order to execute it just by typing the name of the script (here `./myscript.R`) after making it executable (see [here](../linux/index.qmd#permissions-and-ownership) for UNIX-like OSs). The shebang line is optional when executing R scripts from the R console with the `source` command or with `Rscript` from the command-line. The shebang of R scripts looks as follows: 
...
This section has been moved to a dedicated tutorial on R package development [here](https://girke.bioinformatics.ucr.edu/GEN242/tutorials/rpackages/rpackages_index.html).
...
```{r package_skeleton2}
#| eval: false
package.skeleton(name="mypackage", code_files=c("script.R"))
...
See homework section [here](https://girke.bioinformatics.ucr.edu/GEN242/assignments/homework/hw04/hw04.html).
