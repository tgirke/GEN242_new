...
In addition to executing the code in R scripts [interactively](https://girke.bioinformatics.ucr.edu/GEN242/tutorials/rbasics/rbasics/#working-routine-for-tutorials), _e.g._ line by line, one can execute R scripts as a whole with execution commands from the R console or the command-line using `source` or `Rscript`, respectively. The details on how to execute R scripts are explained in [this section](https://girke.bioinformatics.ucr.edu/GEN242/tutorials/rprogramming/rprogramming/#possibilities-for-executing-r-scripts). 
...
The code in R scripts (or Rmd/qmd scripts) should be organized as much as possible in [R functions](https://girke.bioinformatics.ucr.edu/GEN242/tutorials/rprogramming/rprogramming/#functions) that are called (used) somewhere in the script. Alternatively, functions can be organized in a secondary R script from where they are imported with the `source` command. This makes the code more concise, readable and reusable. 
...
__Programming Exercise on Loops:__ go [here](https://girke.bioinformatics.ucr.edu/GEN242/tutorials/rprogramming/rprogramming/#exercise-1)
...
__Programming Exercise on Functions:__ go [here](https://girke.bioinformatics.ucr.edu/GEN242/tutorials/rprogramming/rprogramming/#exercise-2)
https://girke.bioinformatics.ucr.edu/GEN242/tutorials/rprogramming/rprogramming/#exercise-1)
...
A [shebang](https://en.wikipedia.org/wiki/Shebang_(Unix)) needs to be included at the beginning of an R script in order to execute it just by typing the name of the script (here `./myscript.R`) after making it executable (see [here](https://girke.bioinformatics.ucr.edu/GEN242/tutorials/linux/linux/#permissions-and-ownership) for UNIX-like OSs). The shebang line is optional when executing R scripts from the R console with the `source` command or with `Rscript` from the command-line. The shebang of R scripts looks as follows: 
