...
2. Install plugins on first launch. This must be done on the **login node**:
...
3. Log in to a compute node with `srun` and follow the usage instructions in the next section. 
...
since it will create the omni completion database which can take some time since there
are over 1,500 R packages installed on the HPCC cluster. Also, if there any messages
