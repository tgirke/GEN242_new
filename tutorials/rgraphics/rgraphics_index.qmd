...
<p align="right">
  <a href="https://raw.githubusercontent.com/tgirke/GEN242/main/tutorials/rgraphics/rgraphics_index.qmd">
...
quarto render rgraphics_index.qmd
Render qmd from R:
quarto::quarto_render("rgraphics_index.qmd")
-->
...
![sphm](images/grammar_ggplot2.png){fig-align="center"}### `ggplot2` Usage
...
```{r qplot_scatter_plot_dot_param}
...
```{r qplot_scatter_plot_no_legend}
qplot(x, y, geom="point", size=x, color=cat) + 
...
```{r qplot_scatter_plot_colored_groups}
...
```{r qplot_scatter_regression_line}
set.seed(1410)
...
```{r qplot_scatter_regression_loess}
...
Standard R export functons for graphics work as well (see [here](../rgraphics/index.qmd#saving-graphics-to-files)). 

### `ggplot`: scatter plots

#### Basic example

```{r ggplot_scatter_plot1}
set.seed(1410)  
...
```{r ggplot_scatter_plot_interactive1}
library(plotly)
ggplotly(p)
...
```{r ggplot_many_regression_lines}
...
```{r iris_mean_bar_plot_facetting}
#| eval: false
...
![sphm](images/spatial_heatmap.jpeg){fig-align="center"}### Venn Diagrams 
...
```{r ggbio_circular1}
ggbio() + 
  circle(gr, aes(fill = seqnames), geom = "rect")
```
...
![vim-r](images/circularplot.png){fig-align="center"}#### Alignments and variants
...
```{r ggbio_align_variants}
#| warning: false
#| message: false
library(rtracklayer); library(GenomicFeatures); library(txdbmaker); library(Rsamtools); library(GenomicAlignments); library(VariantAnnotation)
...
![vim-r](images/igv_peak_3.png){fig-align="center"}#### Create symbolic links 
