...
**Step 2.** Load `*.qmd`, `*.Rmd` or `*.R` file in Neovim (Nvim-R) or RStudio.

**Step 3.** Send code from code editor to R console by pushing `space bar` in Neovim (Nvim-R) or `Ctrl + Enter` in RStudio. In `*.Rmd` files the code lines are in so called [code chunks](../rmarkdown/index.qmd#r-code-chunks) and only those
