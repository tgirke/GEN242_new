...
subtitle: "T3 — GEN242: Data Analysis in Genome Biology"
author: "Thomas Girke"
date: today
format:
  revealjs:
...
### RStudio / Posit (recommended for beginners)
...
### Nvim-R-Tmux (terminal-based, for HPC users)

Terminal-based environment combining Neovim + R + Tmux. Ideal for working on the HPCC cluster.

- Send line to R console: `Space`
- Start R session: `\rf`
- Full instructions: [Nvim-R-Tmux tutorial](../linux/index.qmd#nvim-r-tmux-essentials)
...
Always write commands into an **R script** (`.R` file) rather than typing directly into the console — this preserves your work and makes it reproducible.

### Step 1 — Download the tutorial source file

```r
download.file(
  "https://raw.githubusercontent.com/tgirke/GEN242//main/content/en/tutorials/rbasics/Rbasics.Rmd",
  "Rbasics.Rmd"
)
```
...
Load the `.Rmd` or `.R` file in RStudio or Neovim.

### Step 3 — Send code to the R console

- **RStudio**: `Ctrl+Enter`
- **Nvim-R**: `Space` (line), `\rf` to start R session first

::: {.callout-note}
In `.Rmd` files, code lives in **code chunks** — only those lines can be sent to the console. In `.R` scripts, any line can be sent.
:::
...
The `%>%` pipe from `dplyr`/`magrittr` chains operations left-to-right:
...
```r
myDF <- read.delim("myData.xls", sep="\t")           # tab-delimited file
```
...
```r
read.delim("clipboard")                   # Windows/Linux: paste into R
read.delim(pipe("pbpaste"))               # macOS: paste into R
write.table(iris, "clipboard", sep="\t", col.names=NA, quote=FALSE)  # copy from R
```
...
[→ HW02 instructions](https://girke.bioinformatics.ucr.edu/GEN242/assignments/homework/hw02/index.html)
:::
...
A step-by-step workflow using two sample biological datasets.
...
```r
my_mw <- read.delim("http://faculty.ucr.edu/~tgirke/Documents/R_BioCond/Samples/MolecularWeight_tair7.xls",
                     header=TRUE, sep="\t")
my_target <- read.delim("http://faculty.ucr.edu/~tgirke/Documents/R_BioCond/Samples/TargetP_analysis_tair7.xls",
                          header=TRUE, sep="\t")
```
...
## Upgrading R and Bioconductor

When upgrading R, **all packages must be reinstalled** — packages are built for specific R versions.

### Step 1 — Export current package list before upgrading
...
### Step 2 — After installing new R, reinstall all packages

```r
install.packages(c("devtools", "tidyverse", "BiocManager"))
BiocManager::install(version="3.19")    # check current version at https://bit.ly/3NADnll
my_R_pkgs <- readLines("my_R_pkgs.txt")
BiocManager::install(my_R_pkgs)
```

::: {.callout-warning}
Packages from private GitHub repos or custom sources cannot be reinstalled this way — identify them from the install report and reinstall manually.
:::
