...
format:
  revealjs:
    ...
    footer: "GEN242 · UC Riverside · [Tutorial](https://girke.bioinformatics.ucr.edu/GEN242/tutorials/rgraphics/rgraphics_index.html)"
...
<center><img title="sphm" src="images/grammar_ggplot2.png" width="400" /></center>
...
```{r ggplot_scatter_plot_interactive1, eval=TRUE, warning=FALSE, message=FALSE, fig.dim=c(5,5), fig.align="center"}
library(plotly)
ggplotly(p)
```
...
```{r ggplot_box_plot2_jitter, eval=TRUE, warning=FALSE, message=FALSE}
...
```{r ggplot_box_plotly_jitter, eval=TRUE, warning=FALSE, message=FALSE, fig.dim=c(4,4), fig.align="center"}
p <- ggplot(dsmall, aes(color, price/carat, fill=color)) + geom_violin() + geom_jitter(aes(color=color))
ggplotly(p) 
```
...
<center><img title="sphm" src="images/spatial_heatmap.jpeg" width="500" /></center>
...
```{r specgraph_structure, eval=TRUE, warning=FALSE, message=FALSE, fig.dim=c(5,5), fig.align="center"}
library(ChemmineR)
data(sdfsample)
...
library(rtracklayer); library(txdbmaker); library(Rsamtools); library(GenomicAlignments); library(VariantAnnotation)
